...
show_menu() {
    clear
    echo -e "${RED}==========================================================${NC}"
    echo -e "${WHITE}          [👹 SHIVAMBROOG LABS PREMIUM VPS DASHBOARD 👹]          ${NC}"
    echo -e "${RED}==========================================================${NC}"
    echo -e "${RED}==========================================================${NC}"
    echo ""
...
create_vps() {
    ...
    if [ ! -f "/home/daytona/ubuntu22.qcow2" ]; then
        ...
        $SUDO_CMD wget -q --show-progress https://cloud.debian.org/images/cloud/bookworm/latest/debian-12-generic-amd64.qcow2 -O /home/daytona/debian12.qcow2
        $SUDO_CMD chmod 666 /home/daytona/debian12.qcow2
    else
        echo -e "${GREEN}✅ Existing Debian Image Cache Detected at /home/daytona/.${NC}"
    fi
...
EOF
    ...
    $SUDO_CMD qemu-img resize /home/daytona/debian12.qcow2 +${DISK_ADD}G > /dev/null 2>&1
...
boot_qemu() {
    ...
    qemu-system-x86_64 \
        -hda /home/daytona/debian12.qcow2 \
        -m $RAM_VALUE \
        -smp ${CPU_CORES:-4} \
...
# RESTART PIPELINE
restart_vps() {
    if [ -f "/home/daytona/debian12.qcow2" ] && [ -f "seed.img" ]; then
...
clean_vps() {
    ...
    $SUDO_CMD rm -rf user-data seed.img /home/daytona/debian12.qcow2 .vps_env
